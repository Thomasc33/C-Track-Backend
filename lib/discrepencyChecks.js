...
module.exports = {
    async check(uid = null) {
        ...
        let asset_tracking_query = await pool.request().query(`SELECT * FROM asset_tracking WHERE date = GETDATE()${uid ? ` AND user_id = '${uid}'` : ''}`)
        ...
        let hourly_tracking_query = await pool.request().query(`SELECT * FROM hourly_tracking WHERE date = GETDATE()${uid ? ` AND user_id = '${uid}'` : ''}`)
            .then(d => d.recordset).catch(er => { console.log(er); return { isErrored: true, error: er } })
        ...
        // Get Snipe Data
        const snipeData = await getSnipeData(today)

        // Get Tsheets Data
        const tsheets_data = await getTsheetsData(job_codes, today, tomorrow)
        ...
        function getUserData(id) {
            ...
            assetJobCodes.forEach(jc => {
                let ts_hours = 0.0, ts_count = 0, count = 0, snipe_count = 0, unique = []
                ...
                if (tsheets_data[today] && tsheets_data[today][id]) for (let i of tsheets_data[today][id].timesheets) if (i.jobCode == `${jc}` || i.jobCode == complimentaryJC) {
                    ts_hours += i.hours;
                ...
                // discrepancy check
                if (job_codes[jc].requires_asset) if ((Object.keys(tsheets_data).length && ts_count !== count) || count !== snipe_count) discrepancies[id].push({ jc, ts_count, count, snipe_count, today, unique })
            })

            hourlyJobCodes.forEach(jc => {
                let ts_hours = 0, ts_count = 0, count = 0
                if (tsheets_data[today] && tsheets_data[today][id]) for (let i of tsheets_data[today][id].timesheets) if (i.jobCode == `${jc}`) { ts_hours += i.hours; ts_count += parseInt(i.count); tsheetsVisited.add(i.id) }
                for (let i of hourly_tracking_query) if (i.user_id == id && i.today.toISOString().split('T')[0] == today && i.job_code == jc) count += i.hours
                if (JobCodePairsSet.has(jc)) {
                    ...
                    if (complimentaryJC) for (let i of d) {
                        ...
                        if (job_codes[`${complimentaryJC}`]) if (i[0].value == job_codes[jc].name && i[0].value == job_codes[`${complimentaryJC}`].name) {
                            ...
                            if (ts_hours != count) discrepancies[id].push({ jc: `${jc} (Hourly)`, ts_hours, count, today, snipe_count: '-' })
                ...
                //discrepancy check
                if (ts_hours !== count) discrepancies[id].push({ jc, ts_hours, count, today, snipe_count: '-' })
            })
            ...
            if (snipeData && snipeData[today] && snipeData[today][id]) {
                for (let i in snipeData[today][id]) {
                    if (!assetJobCodes.has(parseInt(i)) && !assetJobCodes.has(i)) {
                        let ts_count, count = 0, snipe_count = snipeData[today][id][i].length, unique = snipeData[today][id][i].join(', ')
                        if (tsheets_data[today]) for (let i of tsheets_data[today][id].timesheets) if (i.jobCode == i) { ts_count += parseInt(i.count) }
        ...
        async function handleDiscrepancy(id) {
            let values = []
            //{id:[discrepancies]}
            //jc, ts_count, count, snipe_count, today
            for (let i of discrepancies[id]) {
                let job = job_codes[i.jc]
                values.push(`('${id}','1','Discrepency on Job: ${job.name || i.jc}', 'You have a discrepency on the above job code. If you are still working on that code, click X on this notification. C-Track ${isNaN(i.ts_hours) ? 'Hours' : 'Count'}: ${i.count}${isNaN(i.ts_count) ? `, T-Sheets Count: ${i.ts_count}` : ''}${isNaN(i.snipe_count) ? `, Snipe Count: ${i.snipe_count}` : ''}${isNaN(i.ts_hours) ? `, T-Sheets Hours: ${i.ts_hours}` : ''}', GETDATE())`)
            }
        ...
        if (tsheets_data[today]) for (let uid in tsheets_data[today]) for (let sheet of tsheets_data[today][uid].timesheets) {
