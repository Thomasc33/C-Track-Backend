    ...
    async check(uid = null) {
        ...
        async function handleDiscrepancy(id) {
            ...
            for (let i of discrepancies[id]) {
                ...
                values.push(`('${id}','1','Discrepency on Job: ${job.name || i.jc}', 'You have a discrepency on the above job code. If you are still working on that code, click X on this notification. C-Track ${isNaN(i.ts_hours) ? 'Count' : 'Hours'}: ${i.count || 0}${!isNaN(i.ts_count) ? `, T-Sheets Count: ${i.ts_count || 0}` : ''}${!isNaN(i.ts_count) || !job.is_hourly ? `, Snipe Count: ${i.snipe_count || 0}` : ''}${!isNaN(i.ts_hours) ? `, T-Sheets Hours: ${i.ts_hours || 0}` : ''}', GETDATE())`)
            }
