    ...
    async check(uid = null) {
        ...
        function getUserData(id) {
            ...
            if (tsheets_data && tsheets_data[today] && tsheets_data[today][id]) for (let i of tsheets_data[today][id].timesheets) if (i.jobcode_id == tsSettings.CPOCID) {
                ...
                if (jc.includes('overhead') && billable) { discrepancies[id].push({ jc: i.customfields['1164048'], date: today, message: `\`${i.customfields['1164048']}\` is set to billable but should be non-billable` }) }
                else if (jc.includes('professional development') && !billable) { discrepancies[id].push({ jc: i.customfields['1164048'], date: today, message: `\`${i.customfields['1164048']}\` is set to non-billable but should be billable` }) }
            }
        }
