...
module.exports = {
    async check(uid = null) {
        ...
        // Get Tsheets Data
        const tsheets_data = await getTsheetsData(job_codes, today, tomorrow, uid ? [uid] : undefined, true)
        ...
        function getUserData(id) {
            ...
            // if (tsheets_data && tsheets_data[today] && tsheets_data[today][id]) for (let i of tsheets_data[today][id].timesheets) if (i.jobcode_id == tsSettings.CPOCID) {
            //     let billable = i.customfields['1180404'] == 'Yes' ? true : false
            //     let jc = i.customfields['969708'].toLowerCase()
            //     if (jc.includes('overhead') && billable) { discrepancies[id].push({ jc: i.customfields['969708'], date: today, message: `\`${i.customfields['969708']}\` is set to billable but should be non-billable` }) }
            //     else if (jc.includes('professional development') && !billable) { discrepancies[id].push({ jc: i.customfields['969708'], date: today, message: `\`${i.customfields['969708']}\` is set to non-billable but should be billable` }) }
            // }
        }
        ...
        async function handleDiscrepancy(id) {
            ...
            for (let i of discrepancies[id]) {
                ...
                values.push(`('${id}','1','Discrepency on: ${job ? job.name : i.jc}', ${i.message ? `'${i.message}'` : `'C-Track ${isNaN(i.ts_hours) ? 'Count' : 'Hours'}: ${i.count || 0}${!isNaN(i.ts_count) ? `, T-Sheets Count: ${i.ts_count || 0}` : ''}${!isNaN(i.ts_hours) ? `, T-Sheets Hours: ${i.ts_hours || 0}` : ''}'`}, GETDATE())`)
        ...
        if (tsheets_data && tsheets_data[today]) for (let uid in tsheets_data[today]) for (let sheet of tsheets_data[today][uid].timesheets) {
            if (!tsheetsVisited.has(sheet.id) && tsSettings.GentivaCustomerID.split(',').includes(`${sheet.jobcode_id}`)) {
                if (!discrepancies[uid]) discrepancies[uid] = []
                discrepancies[uid].push({ jc: sheet.customfields ? sheet.customfields['969708'] || sheet.notes : sheet.notes, ts_hours: sheet.hours, count: 0, date: today })
            }
        }
