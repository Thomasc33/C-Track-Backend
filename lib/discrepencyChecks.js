...
module.exports = {
    async check() {
        ...
        let asset_tracking_query = await pool.request().query(`SELECT * FROM asset_tracking WHERE date = GETDATE()`)
        ...
        let hourly_tracking_query = await pool.request().query(`SELECT * FROM hourly_tracking WHERE date = GETDATE()`)
            .then(d => d.recordset).catch(er => { console.log(er); return { isErrored: true, error: er } })
