    ...
    async check(uid = null) {
        ...
        function getUserData(id) {
            ...
            hourlyJobCodes.forEach(jc => {
                ...
                // Get Tsheets counts
                if (tsheets_data[today] && tsheets_data[today][id]) for (let i of tsheets_data[today][id].timesheets) if (i.jobCode == `${jc}` || i.jobCode == `${complimentaryJC}`) { ts_hours += i.hours; ts_count += parseInt(i.count); tsheetsVisited.add(i.id) }
        ...
        async function handleDiscrepancy(id) {
            ...
            for (let i of discrepancies[id]) {
                ...
                values.push(`('${id}','1','Discrepency on: ${job ? job.name : i.jc}', 'C-Track ${isNaN(i.ts_hours) ? 'Count' : 'Hours'}: ${i.count || 0}${!isNaN(i.ts_count) ? `, T-Sheets Count: ${i.ts_count || 0}` : ''}${(!isNaN(i.ts_count) || !(job && job.is_hourly)) && i.snipe_count != '-' ? `, Snipe Count: ${i.snipe_count || 0}` : ''}${!isNaN(i.ts_hours) ? `, T-Sheets Hours: ${i.ts_hours || 0}` : ''}', GETDATE())`)
            }
