    ...
    toUID(token) {
        return new Promise(async (res, rej) => {
            ...
            try {
                ...
                //query
                let resu = await pool.request().query(`SELECT id, is_archived FROM users WHERE oid = '${id}'`)
    ...
    checkForAdmin(token) {
        return new Promise(async (res, rej) => {
            ...
            try {
                ...
                //query
                let resu = await pool.request().query(`SELECT id, is_admin, is_archived FROM users WHERE oid = '${id}'`)
                    .catch(er => { return { isErrored: true, error: er } })
                if (resu.rowsAffected == 0) return rej('User not found')
                if (resu.recordset[0].is_archived) return rej({ isErrored: true, error: 'archived' })
