...
const validateToken = async (token) => {
    return new Promise(async (res, rej) => {
        ...
        let q = await axios.get('https://graph.microsoft.com/v1.0/me', {
            ...
            if (er && er.response && er.response.data && er.response.data.error && er.response.data.error.code == 'InvalidAuthenticationToken') return
            console.log(er.response.data, token)
        })
        if (!q || !q.data || !q.data.id) return rej('Invalid token')
        res(q.data.id)
    })
}
...
module.exports = {
    toUID(token) {
        return new Promise(async (res, rej) => {
            ...
            // Validate token
            let id = await validateToken(token).catch(er => false)
            if (!id) return rej({ isErrored: true, error: 'Invalid token' })
    ...
    checkForAdmin(token) {
        return new Promise(async (res, rej) => {
            ...
            try {
                ...
                return rej({ isErrored: true, error: er })
            }
        })
    },
    checkPermissions(token) {
        return new Promise(async (res, rej) => {
            try {
                ...
                if (uid == 'errored') return rej({ isErrored: true, error: isAdmin })
    ...
    getTSheetsToken(token) {
        return new Promise(async (res, rej) => {
            ...
            if (resu.length !== 1) return rej({ isErrored: true, error: `${resu.length} results from uid ${uid}` })

            if (!resu[0].ts_authorization || !resu[0].ts_refresh || !resu[0].ts_expires || !resu[0].ts_uid) return rej({ isErrored: true, error: 'no token info' })
