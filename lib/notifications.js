...
async function checkinNotify(asset, job, user) {
    // Establish SQL Connection
    let pool = await sql.connect(settings.SQLConfig)
    ...
    // Add to notifications table
    for (let i of user_query) {
        await pool.request().query(`INSERT INTO notifications (user_id,title,message,image,important) VALUES ('${i}','Early Checkin','${`${asset} has been checked in early by ${user} for ${job}`}', ${image}, 1)`)
    }
}
