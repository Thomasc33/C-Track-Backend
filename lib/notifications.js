...
async function checkinNotify(asset, job, user, lastUpdate) {
    // Establish SQL Connection
    let pool = await sql.connect(settings.SQLConfig)

    // Get all users with receive_early_checkin_notifications set to true
    let user_query = await pool.request().query(`SELECT id FROM user_permissions WHERE receive_early_checkin_notifications = 1`)
        .catch(er => { console.log(er); return null })
        .then(r => r.recordset.map(m => m.id))
    if (!user_query || !user_query.length) return
    ...
    // Get Image
    let model = await pool.request().query(`SELECT model_number FROM assets WHERE id = '${asset}'`).then(m => m.recordset[0].model_number)
    let image_query = await pool.request().query(`SELECT image FROM models WHERE model_number = '${model}'`)
        .catch(er => { return { isErrored: true, er: er } })
    const image = image_query && image_query.recordset[0] ? `'${image_query.recordset[0].image}'` : 'NULL'

    // Add to notifications table
    for (let i of user_query) await pool.request().query(`INSERT INTO notifications (user_id,title,message,image,important) VALUES ('${i}','Early Checkin','${`${asset} has been checked in early by ${user} for ${job}. It was shipped on ${lastUpdate.toISOString().split('T')[0]}`}', ${image}, 1)`)
}

async function notifyOverride(name, asset, job) {
    // Establish SQL Connection
    let pool = await sql.connect(settings.SQLConfig)

    // Get all users with receive_override_notifications set to true
    let user_query = await pool.request().query(`SELECT id FROM user_permissions WHERE receive_override_notifications = 1`)
        .catch(er => { console.log(er); return null })
        .then(r => r.recordset.map(m => m.id))
    if (!user_query || !user_query.length) return
    ...
    // Add to notifications table
    for (let i of user_query) await pool.request().query(`INSERT INTO notifications (user_id,title,message,image,important) VALUES ('${i}','Override','${`${name} has overridden ${asset} for ${job}`}', ${image}, 1)`)
}
